...
const BlogItem = ({image,title,category,description,id}) => {
  return (
    <div className='border border-black max-w-[330px] sm:max-w-[300px] bg-white hover:shadow-[-7px_7px_0px_#000000]  '>
        <Link href={`/blogs/${id}`}>
            ...
            <p className='mb-3 text-sm tracking-tight text-gray-700' dangerouslySetInnerHTML={{__html: description.slice(0,120)}}></p>
             <Link href={`/blogs/${id}`}>
              <div className='inline-flex items-center py-2 font-semibold text-center'>
                    Read more <Image src={assets.arrow} width={12} alt='' className='ml-2'/>
                </div>
            </Link>
        </div>
    </div>
